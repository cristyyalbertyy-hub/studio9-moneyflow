...
function computeAccountBalancesByCurrency(
  ...
  for (const item of profitDistributions) {
    balances[resolveCurrency(item)] -= Number(item.profitAmount || item.profit_amount || 0);
  }
...
function computeCharityBalancesByCurrency(charityAllocations = [], profitDistributions = []) {
  const balances = emptyCurrencyBalances();
  for (const item of charityAllocations) {
    balances[resolveCurrency(item)] += Number(item.amount || 0);
  }
...
function computeAccountBalance(incomes, expenses, documents, charityAllocations = [], profitDistributions = []) {
...
async function getBootstrapData() {
  ...
  const [expensesRes, incomesRes, documentsRes, categoriesRes, clientsRes, activitiesRes, charityRes, profitRes] =
    await Promise.all([
  ...
  const charityBalances = computeCharityBalancesByCurrency(charityAllocations, profitDistributions);
...
async function getAccountBalance(db, currency = DEFAULT_CURRENCY) {
  let [incomesRes, expensesRes, documentsRes, charityRes, profitRes] = await Promise.all([
    ...
    db.from("profit_distributions").select("profit_amount, currency"),
  ]);
...
async function createProfitDistribution(payload, actorProfile) {
  ...
  await assertCanPay(db, profitAmount, currency);
