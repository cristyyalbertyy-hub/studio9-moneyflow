...
async function getBootstrapData() {
  ...
  let clients = normalizeClients(clientsRes.error ? [] : clientsRes.data || []);
  if (clients.length === 0) {
  ...
  const incomes = mapIncomes(incomesRes.data || []);
...
async function createExpense(payload, actorProfile) {
  ...
  const seqNumber = await getNextExpenseSeqNumber(db);
  ...
  await addActivity({
    ...
    by: actorProfile,
    summary: paid
      ? `Despesa #${seqNumber} registada (ja paga): ${person} — ${formatMoney(amount)} — ${description}`
      : `Nova despesa #${seqNumber} (por pagar): ${person} — ${formatMoney(amount)} — ${description}`,
    meta: { expenseId: id, seqNumber, person, amount, category, paid },
  });
}
...
async function renameClient(oldName, newName) {
  ...
  const incomeUpdate = await db.from("incomes").update({ client: newTrim, source: newTrim }).eq("client", oldTrim);
  throwIfError(incomeUpdate.error);

  const clientUpdate = await db.from("clients").update({ name: newTrim }).eq("name", oldTrim);
  throwIfError(clientUpdate.error);
}
...
async function deleteClient(name) {
  ...
  const inUse = await db.from("incomes").select("id").eq("client", trimmed).limit(1);
  throwIfError(inUse.error);
  if ((inUse.data || []).length > 0) {
...
function isMissingClientsTable(error) {
  ...
  return message.includes("clients") && (message.includes("does not exist") || message.includes("could not find"));
}
