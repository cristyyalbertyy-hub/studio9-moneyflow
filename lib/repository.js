...
async function createExpense(payload, actorProfile) {
  ...
  let insertRes = await db.from("expenses").insert({ ...baseRow, seq_number: seqNumber });
  if (insertRes.error && isMissingSeqNumberColumn(insertRes.error)) {
    insertRes = await db.from("expenses").insert(baseRow);
    seqNumber = null;
  }
  ...
  await addActivity({
    ...
    by: actorProfile,
    summary: paid
      ? `Despesa ${seqLabel}registada (ja paga): ${person} — ${formatMoney(amount)} — ${description}`
      : `Nova despesa ${seqLabel}(por pagar): ${person} — ${formatMoney(amount)} — ${description}`,
