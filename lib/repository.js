...
function computeAccountBalancesByCurrency(incomes, expenses, documents, charityAllocations = []) {
  ...
  for (const item of documents) {
    if (!item.paid) continue;
    balances.EUR -= Number(item.amount || 0);
  }
...
function computeCharityBalancesByCurrency(charityAllocations = []) {
  const balances = emptyCurrencyBalances();
  for (const item of charityAllocations) {
    balances[resolveCurrency(item)] += Number(item.amount || 0);
  }
...
function computeAccountBalance(incomes, expenses, documents, charityAllocations = []) {
  return computeAccountBalancesByCurrency(incomes, expenses, documents, charityAllocations).EUR;
}

async function getBootstrapData() {
  const db = getDb();

  const [expensesRes, incomesRes, documentsRes, categoriesRes, clientsRes, activitiesRes, charityRes] =
    await Promise.all([
  ...
  const charityBalances = computeCharityBalancesByCurrency(charityAllocations);
  ...
  return {
    ...
    summary: {
      accountBalance: computeAccountBalance(incomes, expenses, documents, charityAllocations),
      accountBalances: computeAccountBalancesByCurrency(incomes, expenses, documents, charityAllocations),
      charityBalance: charityBalances.EUR,
      charityBalances,
...
async function getAccountBalance(db, currency = DEFAULT_CURRENCY) {
  let [incomesRes, expensesRes, documentsRes, charityRes] = await Promise.all([
  ...
  const balances = computeAccountBalancesByCurrency(incomes, expenses, documents, charityAllocations);
