...
const RESET_CONFIRM_PHRASE = "REPOS";
...
function requireSharedPassword(password) {
  const { authDisabled, password: sharedPassword } = getAuthConfig();
...
function requireResetConfirmation(confirmPhrase) {
  if (String(confirmPhrase || "").trim().toUpperCase() !== RESET_CONFIRM_PHRASE) {
    const err = new Error("Confirmacao invalida. Escreve REPOS para continuar.");
    err.status = 400;
    throw err;
  }
}
